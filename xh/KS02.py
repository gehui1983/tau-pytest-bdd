...
# 结算   /home/james/Documents/2025.2.20原始数据/KS/快手-善行-结算单.xlsx_182382610_1.xlsx
# 获取 订单号和实际结算金额(元)
def settlement_process(file_name: str) -> dict:
    order_dict = dict()
    excel = pd.ExcelFile(file_name)
    pending_pd = pd.read_excel(excel, dtype={"订单号": str, "实际结算金额(元)": str})
    for t in pending_pd.iterrows():
        index, row = t
        order_number = str(row["订单号"]).strip()
        expected = str(row["实际结算金额(元)"]).strip()
        if not is_float(expected):
            expected = "0.00"
        values = order_dict.get(order_number)
        if values is None:
            ...
            assert isinstance(values, list)
            values.append(expected)
    print("settlement_process len:", len(order_dict.keys()))
    return order_dict
